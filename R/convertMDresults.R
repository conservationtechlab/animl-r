  ...
  for(i in 1:length(mdresults)){
    ...
    if(class(mdresults[[i]]$detections)=="data.frame"){
      ...
      for(j in 1:length(s)){
        ...
        line<-data.frame(image_path=mdresults[[i]]$file,md_class=s[j,]$category,md_confidence=s[j,]$conf,pixelx=jpgx,pixely=jpgy,
                         x1=s[j,]$bbox1,x2=s[j,]$bbox2,y1=s[j,]$bbox3,y2=s[j,]$bbox4,
                         xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax)
        images_flat<-rbind(images_flat,line)
      }
    }else{
      line<-data.frame(image_path=mdresults[[i]]$file,md_class=0,md_confidence=mdresults[[i]]$max_detection_conf,pixelx=jpgx,pixely=jpgy,
...
convertMDResultsSimple<-function(mdresults){
  f<-function(data){
    ...
    }else{data.frame(file=data$file,max_detection_conf=data$max_detection_conf,max_detection_category=data$max_detection_category,category=0,conf=NA,bbox1=NA,bbox2=NA,bbox3=NA,bbox4=NA,stringsAsFactors = F)}
  }
