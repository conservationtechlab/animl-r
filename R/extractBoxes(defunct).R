...
extractBoxes <- function(image, min_conf = 0, buffer = 2, plot = TRUE, return.crops = FALSE, save = FALSE, resize = NA, outdir = "", quality = 0.8) {
  ...
  images_flat <- data.frame(
    image_path = character(), md_class = numeric(), md_confidence = numeric(), pixelx = numeric(), pixely = numeric(),
  ...
  if (is.data.frame(image$detections)) {
    ...
    for (j in 1:nrow(s)) {
      ...
      line <- data.frame(
        image_path = image$file, md_class = as.numeric(s[j, ]$category), md_confidence = s[j, ]$conf, pixelx = jpgx, pixely = jpgy,
    ...
    line <- data.frame(
      image_path = image$file, md_class = 0, md_confidence = image$max_detection_conf, pixelx = jpgx, pixely = jpgy,
...
extractAllBoxes <- function(images, min_conf = 0, buffer = 2, save = FALSE, resize = NA, outdir = "", quality = 0.8, parallel = FALSE, nproc = parallel::detectCores()) {
  ...
  run.parallel <- function(i) {
    if (file.exists(images[[i]]$file)) {
      extractBoxes(images[[i]], min_conf = min_conf, buffer = buffer, resize = resize, save = save, outdir = outdir, plot = F, quality = quality)
    } else {
      NA
    }
  }
  ...
  if (parallel) {
    ...
    parallel::clusterExport(cl, list("buffer", "resize", "quality", "outdir", "images", "extractBoxes", "resize_pad"), envir = environment())
...
extractAllBoxesFromFlat <- function(images, buffer = 0, resize = NA, quality = 0.8, outdir = "", save = FALSE, overwrite = TRUE, parallel = FALSE, nproc = parallel::detectCores()) {
  ...
  if (overwrite == T) {
    run.parallel <- function(i) {
      if (file.exists(images[i, ]$image_path)) {
        extractBoxesFromFlat(images[i, ], buffer = buffer, resize = resize, save = save, outdir = outdir, plot = F, quality = quality)
      } else {
        NA
      }
    }
    ...
    run.parallel <- function(i) {
      if (file.exists(images[i, ]$image_path) & !file.exists(paste0(outdir, images[i, ]$crop_rel_path))) {
        extractBoxesFromFlat(images[i, ], buffer = buffer, resize = resize, save = save, outdir = outdir, plot = F, quality = quality)
      } else {
        NA
      }
  ...
  if (parallel) {
    ...
    cl <- parallel::makeCluster(min(parallel::detectCores(), nproc), type = type)
    # clusterExport(cl, expList <- as.list(objects(pos = globalenv())))
    parallel::clusterExport(cl, list("buffer", "resize", "quality", "outdir", "images", "extractBoxesFromFlat", "resize_pad"), envir = environment())
    # set random number generator for cluster
    parallel::clusterSetRNGStream(cl)
