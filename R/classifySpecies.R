...
#' @param models models with which to classify species
...
classifySpecies<-function(mdresults,model,resize=299,standardize=TRUE,batch_size=32,workers=1){
  ...
  predlist<-model %>% predict_generator(generator=cropGenerator,steps=predict_steps,workers=workers,verbose=1)
  return(predlist)
}
